// Brain Rot Assessment App - Performance Optimized

// Performance monitoring
const startTime = performance.now();

// Critical performance optimization: Preload key resources
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        // Use relative path for better compatibility with different hosting platforms
        navigator.serviceWorker.register('sw.js').catch(() => {});
        
        // Log performance metrics
        const loadTime = performance.now() - startTime;
        if (loadTime > 1000) {
            console.warn('⚠️ Slow load time detected:', loadTime + 'ms');
        }
    });
}

// Defer non-critical operations with fallback
const deferredInit = () => {
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Initializing Brain Rot Assessment app...');
...
function optimizeForMobile() {
    ...
    style.textContent = `
        @media (max-width: 768px) {
            ...
            .brain-icon { animation: none; }
            .emoji-float { animation: none; }
            
            /* Ensure newsletter forms are always visible */
            .beehiiv-container, .beehiiv-container-results {
    ...
    // Fix newsletter iframe rendering issues
    setTimeout(() => {
        const iframes = document.querySelectorAll('.beehiiv-embed');
        iframes.forEach(iframe => {
            ...
            iframe.addEventListener('load', function() {
                this.style.minHeight = window.innerWidth <= 320 ? '750px' : 
                                      window.innerWidth <= 414 ? '700px' : '650px';
            });
        });
    }, 1000);
...
function setupNewsletterForm() {
    // Handle any custom newsletter form if present
...
function setupScrollEffects() {
    ...
    const elements = document.querySelectorAll('.content-card, .article-card, .resource-category, .faq-item');
...
function setupAdvancedAnimations() {
    ...
    style.textContent = `
        ...
        .content-card, .article-card, .resource-category, .faq-item { 
            opacity: 0; 
...
// FAQ Functions with improved accessibility
function populateFAQs() {
    ...
    faqs.forEach((faq, index) => {
        ...
        faqQuestion.addEventListener('click', function() {
            ...
            faqQuestion.setAttribute('aria-expanded', isActive.toString());
        });
